...
def load_datasets_from_results_csv(results_csv_path):
    ...
    for _, row in results_df.iterrows():
        ...
        datasets.append({
            ...
            'file_path': file_path,
            'dataset_type': 'static'  # Assume all datasets in the directory are static
        })
...
if st.session_state.wen_datasets:
    for index, pivot_table in enumerate(st.session_state.wen_datasets):
        render_interface_for_table(pandas_ai, index, pivot_table)
...
with col2:
    ...
    if user_input:
        with chat_container:
            ...
            route = get_question_route(pandas_ai, user_input)

            if route['request_type'] == 'WEN-KEN database':
                refined_request = get_refined_question(pandas_ai, user_input)
                if refined_request['is_request_data']:
                    plan = get_request_plan(pandas_ai, refined_request['request'])
                ...
                message = process_regulation_request(pandas_ai, user_input, chat_container)
                st.chat_message("assistant").markdown(message)
                st.session_state.chat.append({"role": "assistant", "content": message})
                st.rerun()
            elif route['request_type'] == 'Data Commons':
                code = process_data_commons_request(pandas_ai, user_input, st.session_state.datasets)
                ...
                message = process_off_topic_request(pandas_ai, user_input, chat_container)
                st.chat_message("assistant").markdown(message)
                st.session_state.chat.append({"role": "assistant", "content": message})
                st.rerun()
